...
//防止内存泄露的点击事件
fun <T> View.weakClick(host:T,onClick: (T) -> Unit): View {
    val weakObj = WeakReference(host)
    RxView.clicks(this)
            .throttleFirst(500, TimeUnit.MILLISECONDS)
            .subscribe {
                val sObj = weakObj.get()
                if(sObj != null) { onClick.invoke(sObj!!) }
            }
    return this
}

//长点击事件
fun <T> View.weakLongClick(host:T,onClick: (T) -> Unit): View {
    val weakObj = WeakReference(host)
...
fun TextView.leftDrawable(resId: Int, paddingVal: Int = 0, width: Int = 0, height: Int = 0) {
    ...
    drawable.setBounds(0, 0, if (width > 0) width else drawable.minimumWidth, if (height > 0) height else drawable.minimumHeight)
...
fun TextView.rightDrawable(resId: Int, paddingVal: Int = 0, width: Int = 0, height: Int = 0) {
    val drawable = resources.getDrawable(resId)
    drawable.setBounds(0, 0, if (width > 0) width else drawable.minimumWidth, if (height > 0) height else drawable.minimumHeight)
    setCompoundDrawables(null, null, drawable, null)
    compoundDrawablePadding = paddingVal
...
fun TextView.topDrawable(resId: Int, paddingVal: Int = 0, width: Int = 0, height: Int = 0) {
    val drawable = resources.getDrawable(resId)
    drawable.setBounds(0, 0, if (width > 0) width else drawable.minimumWidth, if (height > 0) height else drawable.minimumHeight)
